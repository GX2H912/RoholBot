...
@bot.slash_command(
    name='rohol_qotd',
    description='Rohol quote(s) of the day.'
)
async def rohol_qotd(
        ctx: discord.context,
        show: Option(bool, 'Show quote(s) in chat?', default=False),
        n: Option(int, 'Number of quotes', default=1),
) -> None:
    def get_quote():
    ...
    fields = (
        [
            f'The \'{adjective()}\' Rohol Quote{" (" + str(i + 1) + ")" if n > 1 else ""}:\n',
            f'{get_quote()}\n-Brent Rohol {date}'
        ]
        for i in range(n)
    )
